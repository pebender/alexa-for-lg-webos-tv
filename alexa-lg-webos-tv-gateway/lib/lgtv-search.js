        ...
        function ssdpProcess(type, headers, rinfo, callback) {
            ...
            http.get(headers.LOCATION).then((descriptionXml) => {
                xml2js(descriptionXml.data, (error, description) => {
                    ...
                    // check anyway.
                    if (!description.hasOwnProperty('root') ||
                        !description.root.hasOwnProperty('device') ||
                        description.root.device.length != 1 ||
                        !description.root.device[0].hasOwnProperty('manufacturer') ||
                        description.root.device[0].manufacturer.length != 1 ||
                        !description.root.device[0].hasOwnProperty('friendlyName') ||
                        description.root.device[0].friendlyName.length != 1 ||
                        !description.root.device[0].hasOwnProperty('UDN') ||
                    ...
                    tv.name = description.root.device[0].friendlyName[0];
                    tv.udn = description.root.device[0].UDN[0];
                    // Get the mac address needed to turn on the TV using wake on
                    // lan.
                    arp.getMAC(tv.ip, (error, mac) => {
                        if (error) {
                            callback(error, null);
                            return error;
                        }
